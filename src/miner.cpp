...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zbtks that have been in mempool long
                    //and higher priority to zbtks that are large in value
                    ...
                    // zBTK spends can have very large priority, use non-overflowing safe functions.
        ...
        while (!vecPriority.empty()) {
            ...
            // Double check that there are no double spent zBTK spends in this block or tx.
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                // This zBTK serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        if (nHeight > Params().LAST_POW_BLOCK()) {
            ...
            if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
                ...
                // For the period before v2 activation, zBTK will be disabled and previous block's checkpoint is all that will be needed.
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake())
            pwalletMain->zbtkTracker->RemovePending(pblock->vtx[1].GetHash());
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zBTK with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    continue;
                }

                // Sign block with the zBTK key.
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("BitcoinMiner(): Signing new block with zBTK key failed \n");
                    continue;
                }
