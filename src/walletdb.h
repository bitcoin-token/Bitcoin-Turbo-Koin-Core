...
public:
    ...
    bool WriteZBTKSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBTKSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBTKSeed_deprecated(uint256& seed);
    bool EraseZBTKSeed();
    bool EraseZBTKSeed_deprecated();

    bool WriteZBTKCount(const uint32_t& nCount);
    bool ReadZBTKCount(uint32_t& nCount);
