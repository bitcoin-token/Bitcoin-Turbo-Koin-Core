...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the BTK and zBTK money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-zbtkstake=<n>", strprintf(_("Enable or disable staking functionality for zBTK inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzbtk=<n>", strprintf(_("Enable automatic wallet backups triggered after each zBTK minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zbtkbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zBTK backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizebtkamount=<n>     " + strprintf(_("Keep N BTK anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The BTK Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. BTK Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. BTK Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //BTK: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //BTK specific: zerocoin and spork DB's
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZBTKMinted();
                        RecalculateZBTKSpent();
                    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of BTK Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart BTK Core to complete") << "\n";
        ...
        zwalletMain = new CzBTKWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zBTKWallet
        uiInterface.InitMessage(_("Syncing zBTK wallet..."));

        bool fEnableZBTKBackups = GetBoolArg("-backupzbtk", true);
        pwalletMain->setZBTKAutoBackups(fEnableZBTKBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zbtkTracker->Init();
