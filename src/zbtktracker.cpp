...
CzBTKTracker::CzBTKTracker(std::string strWalletFile)
{
...
CzBTKTracker::~CzBTKTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBTKTracker::Init()
{
...
bool CzBTKTracker::Archive(CMintMeta& meta)
{
...
bool CzBTKTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBTKTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBTKTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBTKTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBTKTracker::GetSerialHashes()
{
...
CAmount CzBTKTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBTKTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBTKTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBTKTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBTKTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBTKTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBTKTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBTKTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBTKTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBTKTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBTKTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBTKWallet* zBTKWallet)
{
    bool iszBTKWalletInitialized = (NULL != zBTKWallet);
    CMintMeta meta;
    ...
    if (! iszBTKWalletInitialized)
        zBTKWallet = new CzBTKWallet(strWalletFile);
    meta.isSeedCorrect = zBTKWallet->CheckSeed(dMint);
    if (! iszBTKWalletInitialized)
        delete zBTKWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBTKTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBTKTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBTKTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBTKTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBTKTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBTKTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBTKWallet* zBTKWallet = new CzBTKWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zBTKWallet);
        }
        delete zBTKWallet;
        LogPrint("zero", "%s: added %d dzbtk from DB\n", __func__, listDeterministicDB.size());
    }
