...
# operating on affine and Btkobian coordinates, including the point at infinity
...
def law_btkobian_weierstrass_add(A, B, pa, pb, pA, pB, pC):
  """Check whether the passed set of coordinates is a valid Btkobian add, given assumptions"""
...
def law_btkobian_weierstrass_double(A, B, pa, pb, pA, pB, pC):
  """Check whether the passed set of coordinates is a valid Btkobian doubling, given assumptions"""
  assumeLaw = (good_affine_point(pa) +
               good_affine_point(pb) +
               good_btkobian_point(pA) +
               good_btkobian_point(pB) +
...
def check_exhaustive_btkobian_weierstrass(name, A, B, branches, formula, p):
  """Verify an implementation of addition of Btkobian points on a Weierstrass curve, by executing and validating the result for every possible addition in a prime field"""
  F = Integers(p)
...
def check_symbolic_btkobian_weierstrass(name, A, B, branches, formula):
  """Verify an implementation of addition of Btkobian points on a Weierstrass curve symbolically"""
