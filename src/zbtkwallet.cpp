...
CzBTKWallet::CzBTKWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbtk seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBTKSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBTKSeed_deprecated()) {
                    LogPrintf("%s: Updated zBTK seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbtk seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbtk wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBTKWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBTKCount(nCountLastUsed);
    else if (!walletdb.ReadZBTKCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBTKWallet::Lock()
{
    seedMaster = 0;
}

void CzBTKWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBTKWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBTK(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBTKWallet::LoadMintPoolFromDB()
{
...
void CzBTKWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBTKWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBTKWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbtkTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBTKWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBTK(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbtkTracker which also adds to database
    pwalletMain->zbtkTracker->Add(dMint, true);
...
void CzBTKWallet::SeedToZBTK(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBTKWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBTKWallet::GenerateDeterministicZBTK(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBTKWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBTKWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBTKWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
