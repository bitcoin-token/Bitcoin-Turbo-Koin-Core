...
// Possible states for zBTK send
enum ZerocoinSpendStatus {
    ZBTK_SPEND_OKAY = 0,                            // No error.
    ZBTK_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text.
    ZBTK_WALLET_LOCKED = 2,                         // Wallet was locked.
    ZBTK_COMMIT_FAILED = 3,                         // Commit failed, reset status.
    ZBTK_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed.
    ZBTK_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed.
    ZBTK_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds.
    ZBTK_TRX_CREATE = 7,                            // Everything related to create the transaction.
    ZBTK_TRX_CHANGE = 8,                            // Everything related to transaction change.
    ZBTK_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn.
    ZBTK_INVALID_COIN = 10,                         // Selected mint coin is not valid.
    ZBTK_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness.
    ZBTK_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify.
    ZBTK_BAD_SERIALIZATION = 13,                    // Transaction verification failed.
    ZBTK_SPENT_USED_ZBTK = 14,                      // Coin has already been spend.
    ZBTK_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size.
    ZBTK_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100.
};
...
public:
    ...
    void ZBTKBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZBTKOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzBTKWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzBTKTracker> zbtkTracker;
    ...
    void setZWallet(CzBTKWallet* zwallet)
    {
        zwalletMain = zwallet;
        zbtkTracker = std::unique_ptr<CzBTKTracker>(new CzBTKTracker(strWalletFile));
    }

    CzBTKWallet* getZWallet() { return zwalletMain; }
    ...
    void setZBTKAutoBackups(bool fEnabled)
    {
    ...
    /** zBTK reset */
    boost::signals2::signal<void()> NotifyzBTKReset;
