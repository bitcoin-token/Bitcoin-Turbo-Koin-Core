                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zBTK ought to be enough for anybody." - Bill Gates, 2017
    ui->zBTKpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBTKSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBTKpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBTK_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint zBTK"),
                                 tr("zBTK is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zBTK, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBTK...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBTK in ") +
...
void PrivacyDialog::on_pushButtonSpendzBTK_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint zBTK"),
                                 tr("zBTK is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zBTK, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zBTK
        sendzBTK();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBTK
    sendzBTK();
}

void PrivacyDialog::on_pushButtonZBTKControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZBTKControlDialog* zBTKControl = new ZBTKControlDialog(this);
    zBTKControl->setModel(walletModel);
    zBTKControl->exec();
}

void PrivacyDialog::setZBTKControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBTKSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBTK()
{
    ...
    if(ui->payTo->text().isEmpty()){
        QMessageBox::information(this, tr("Spend zBTK"), tr("No 'Pay To' address provided, creating local payment"), QMessageBox::Ok, QMessageBox::Ok);
    }
    else{
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend zBTK"), tr("Invalid BTK Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBTKpayAmount->text().toDouble();
    CAmount nAmount = roundint64(dAmount* COIN);

    // Check amount validity
    if (!MoneyRange(nAmount) || nAmount <= 0.0) {
        QMessageBox::warning(this, tr("Spend zBTK"), tr("Invalid Send Amount"), QMessageBox::Ok, QMessageBox::Ok);
        ui->zBTKpayAmount->setFocus();
        return;
    }

    // Convert change to zBTK
    ...
    // Warn for additional fees if amount is not an integer and change as zBTK is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BTK </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBTK</b>";
    ...
    // use mints from zBTK selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBTKControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZBTKControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend zBTK"), tr("Version 1 zBTK require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zBTK"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZBTK_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend zBTK"), tr("Version 1 zBTK require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zBTK"));
            ui->TEMintStatus->repaint();
            return;
        ...
        if (nNeededSpends > nMaxSpends) {
            ...
            QMessageBox::warning(this, tr("Spend zBTK"), strStatusMessage.toStdString().c_str(), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Spend zBTK failed with status = ") +QString::number(receipt.GetStatus(), 10) + "\n" + "Message: " + QString::fromStdString(strStatusMessage.toStdString()));
        }
        else {
            QMessageBox::warning(this, tr("Spend zBTK"), receipt.GetStatusMessage().c_str(), QMessageBox::Ok, QMessageBox::Ok);
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zBTK was spent successfully update the addressbook with the label
    ...
    // Clear zbtk selector in case it was used
    ZBTKControlDialog::setSelectedMints.clear();
    ui->labelzBTKSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBTK Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BTK, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBTK Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zbtkTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBTK </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBTK "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBTK "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zBTK "));
    ui->labelzBTKAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zBTK </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zBTK </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zBTK </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzBTKSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzBTK->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zBTK
        ui->pushButtonMintzBTK->setEnabled(false);
        ui->pushButtonMintzBTK->setToolTip(tr("zBTK is currently disabled due to maintenance."));

        // Spend zBTK
        ui->pushButtonSpendzBTK->setEnabled(false);
        ui->pushButtonSpendzBTK->setToolTip(tr("zBTK is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zBTK
        ui->pushButtonMintzBTK->setEnabled(true);
        ui->pushButtonMintzBTK->setToolTip(tr("PrivacyDialog", "Enter an amount of BTK to convert to zBTK", 0));

        // Spend zBTK
        ui->pushButtonSpendzBTK->setEnabled(true);
        ui->pushButtonSpendzBTK->setToolTip(tr("Spend zBTK. Without 'Pay To:' address creates payments to yourself."));
    }
}
