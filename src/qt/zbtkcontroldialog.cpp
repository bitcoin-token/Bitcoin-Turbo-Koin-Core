...
#include "ui_zbtkcontroldialog.h"
...
std::set<std::string> ZBTKControlDialog::setSelectedMints;
std::set<CMintMeta> ZBTKControlDialog::setMints;

bool CZBTKControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZBTKControlDialog::COLUMN_DENOMINATION || column == ZBTKControlDialog::COLUMN_VERSION || column == ZBTKControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZBTKControlDialog::ZBTKControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZBTKControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZBTKControlDialog::~ZBTKControlDialog()
{
    delete ui;
}

void ZBTKControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBTKControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZBTKControlWidgetItem* itemDenom(new CZBTKControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZBTKControlWidgetItem *itemMint = new CZBTKControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if (!mint.isSeedCorrect)
                strReason = "The zBTK seed used to mint this zBTK is not the same as currently hold in the wallet";
...
void ZBTKControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBTKControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBTK_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBTKControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBTKControlDialog::GetSelectedMints()
{
...
void ZBTKControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
